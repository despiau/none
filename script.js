...
micButton.addEventListener('click',async e=>{
  ...
  if(!isRecording){
    ...
    micButton.innerHTML='🔴'
    liveTranscriptElement.innerText='Listening...'
  } else {
    stopRecording()
    commitAudio()
    micButton.classList.remove('recording')
    micButton.innerHTML='🎤'
    liveTranscriptElement.innerText='Processing...'
  }
...
function connectRealtimeAPI(){
  ...
  ws.onopen=()=>{
    ws.send(JSON.stringify({
      type:"session.update",
      session:{
        instructions:"You are a helpful and friendly AI assistant. Respond in English with a lively and playful tone.",
        voice:"alloy",
  ...
  if(event.type==="conversation.item.created"){
    ...
    if(item.role==="assistant"){
      ...
      if(textContent){
        ...
        liveTranscriptElement.innerText='Ready'
      }
